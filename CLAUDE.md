...
- คอมมิตล่าสุดใน repo นี้ (branch `main`): **2026-07-21**
...
- **2026-07-21 เพิ่มฟีเจอร์ "Office Asset Scan"**: หน้าใหม่ `office.html` (clone จาก `index.html`) + คอลัมน์ `department` ใน `assets`/`scan_logs`/`unmatched_assets` (ดู `migration_office_department.sql`) ให้ auditor ระบุแผนกตอนสแกนของในออฟฟิศได้ — **หมายเหตุ: ตอนนี้มีแค่ "แผนก" เป็น free text เท่านั้น ยังไม่มีการระบุ "User/เจ้าของ asset" ตามที่เคยคุยออกแบบไว้ (ดูหัวข้อ 6 ข้อ 9)**
...
├── office.html                     # Frontend: แอพสแกนของในออฟฟิศ (static, ~116KB) — clone จาก index.html เพิ่ม field "แผนก" (ดูข้อ 6.9 เรื่อง maintenance burden)
├── migration_office_department.sql # Migration เพิ่มคอลัมน์ department (assets/scan_logs/unmatched_assets) สำหรับ Office Asset Scan ต้องรันเองใน Neon ก่อน deploy main.py เวอร์ชันที่ใช้คอลัมน์นี้
...
9. **`office.html` เป็นไฟล์ copy เต็มจาก `index.html` ไม่ได้แชร์โค้ดกัน (maintenance burden จริงตามที่เคยเตือนไว้)** — บั๊กหรือ fix ที่เกิดกับ scan logic ใน `index.html` (เช่น guard กันสแกนซ้ำ, offline queue, Cloudinary upload) จะไม่ถูกแก้ใน `office.html` โดยอัตโนมัติ ต้องไล่แก้ซ้ำสองที่เอง — ยังไม่ได้ refactor เป็น shared JS module ตามแผนที่เคยคุยไว้ (ควรทำถ้ามีเวลา เพื่อลดความเสี่ยงบั๊กหลุดจุดใดจุดหนึ่ง) นอกจากนี้ Office Asset Scan ที่ implement จริงมีแค่ "แผนก" (`department`, free text พร้อม auto-fill/mismatch detection จาก `scanned_department`/`dept_mismatch` ใน `scan_logs`) **ยังไม่มีการระบุ "User/เจ้าของ asset ในออฟฟิศ" ตามที่เคยออกแบบคุยไว้** ถ้าจะเพิ่มทีหลังต้องคิด schema ใหม่ (ตอนนั้นคุยไว้ว่าอยากให้เป็น dropdown จาก master list ไม่ใช่ free text)
10. **เหตุการณ์ 2026-07-21: production พังเพราะ push ตรงเข้า `main` โดยไม่ผ่านการตรวจสอบ (เกิดซ้ำหลายรอบ)** — ระหว่างการเพิ่มฟีเจอร์ Office Asset Scan มีการ push ไฟล์ (`main.py`, `dashboard.html`, `migration_office_department.sql`) ทับด้วยเนื้อหา placeholder/ไม่ครบโดยไม่ได้ตั้งใจ **อย่างน้อย 4 ครั้ง** (ดู commit message `"Fix: restore ... (previous push accidentally used placeholder)"` ใน git history) ครั้งที่ร้ายแรงที่สุดคือการแก้ popup "Bata Loading" ค้าง (เพิ่ม `hideBataLoading()` ใน `doLogin()`) แต่ดันลบคำว่า `doLogin()` ออกจาก `onclick`/`onkeydown` ทั้ง 4 จุดในหน้า login ไปด้วยโดยไม่ได้ตั้งใจ ทำให้เกิด `SyntaxError` และ**ปุ่ม Sign in ใช้งานไม่ได้เลยบน production จริง** ก่อนจะแก้กลับ — Railway และ GitHub Pages ทั้งคู่ auto-deploy จาก `main` ทันทีที่ push จึงไม่มี buffer ให้ตรวจสอบก่อน **ควรพิจารณา deploy จาก branch แยก (เช่น `staging`) แล้วค่อย merge เข้า `main` หลังทดสอบแล้วว่าใช้งานได้จริง** เพื่อลดความเสี่ยงแบบนี้ในอนาคต
...
- **2026-07-21** — เพิ่มฟีเจอร์ **Office Asset Scan**: สร้างหน้าใหม่ `office.html` (clone จาก `index.html` ทั้งไฟล์ ไม่ได้แชร์โค้ดกัน — ดูหัวข้อ 6.9) + migration `migration_office_department.sql` เพิ่มคอลัมน์ `department`/`scanned_department`/`dept_mismatch`/`dept_note`/`department_guess` ให้ auditor ระบุแผนกตอนสแกนของในออฟฟิศได้ (free text, ยังไม่ใช่ dropdown จาก master list ตามที่เคยคุยออกแบบไว้ และยังไม่มีการระบุ "User/เจ้าของ asset")
  - **เหตุการณ์ไม่คาดคิดระหว่างทำงาน (สำคัญ):** ระหว่าง session นี้เกิดการ push ทับไฟล์ด้วย placeholder/เนื้อหาไม่ครบ **อย่างน้อย 4 ครั้ง** ใน `main.py`, `dashboard.html`, และ `migration_office_department.sql` (ไม่ได้ผ่าน Claude — push ตรงเข้า `main` จากที่อื่น) ต้องแก้กลับหลายรอบ ครั้งที่ร้ายแรงที่สุดคือการแก้ popup "Bata Loading" ค้างตอน login (เพิ่ม `hideBataLoading()` ถูกต้อง) แต่ push พลาดจนคำว่า `doLogin()` หายไปจาก `onclick`/`onkeydown` ทั้ง 4 จุดในฟอร์ม login ของ `dashboard.html` ทำให้เกิด `SyntaxError` และ **ปุ่ม Sign in ใช้งานไม่ได้เลยบน production จริงชั่วขณะหนึ่ง** ก่อนที่ Claude จะตรวจพบและแก้กลับให้ครบ (คืนค่า `doLogin()` ทั้ง 4 จุด พร้อมตรวจ syntax ผ่าน `node --check` ก่อน push) ดูรายละเอียดที่หัวข้อ 6.10 — บทเรียน: ควร deploy ผ่าน branch แยกก่อน merge เข้า `main` เพื่อลดความเสี่ยงนี้ในอนาคต

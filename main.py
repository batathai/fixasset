...
def assets_by_branch(branch_id: str, db=Depends(get_db), user=Depends(get_current_user)):
    cur = db.cursor()
    cur.execute("SELECT id, qr_key, asset_code, seq, name, serial_no, purchase_date, status FROM assets WHERE location_code = %s ORDER BY asset_code, seq", (branch_id,))
...
def create_session(req: SessionCreate, db=Depends(get_db), user=Depends(get_current_user)):
    ...
    cur.execute("SELECT id FROM audit_sessions WHERE branch_id = %s AND audit_date = %s AND status = 'open'", (req.branch_id, req.audit_date))
...
def create_scan(req: ScanLogCreate, db=Depends(get_db), user=Depends(get_current_user)):
    ...
    if cur.fetchone():
        raise HTTPException(status_code=409, detail="Asset นี้ถูก scan แล้วในวันนี้ (branch เดียวกัน)")
    # เช็คซ้ำใน session เดิม
...
def dashboard_summary(db=Depends(get_db), user=Depends(get_current_user)):
    ...
    return {"total_assets": total_assets, "pending_unmatched": pending_unmatched, "sessions": sessions_data}

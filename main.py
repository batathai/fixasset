...
def assets_by_branch(branch_id: str, db=Depends(get_db), user=Depends(get_current_user)):
    ...
    cur.execute("""
        SELECT id, qr_key, asset_code, seq, name, serial_no,
               purchase_date, status, location_code, department
        FROM assets
    ...
    if len(rows) < 10:
        cur.execute("""
            SELECT DISTINCT a.id, a.qr_key, a.asset_code, a.seq, a.name,
                   a.serial_no, a.purchase_date, a.status, a.location_code, a.department
            FROM assets a
...
@app.get("/assets/branch/{branch_id}/departments")
def departments_by_branch(branch_id: str, db=Depends(get_db), user=Depends(get_current_user)):
    """รายชื่อแผนกทั้งหมดที่มีจริงใน asset master ของ location นี้ — ใช้ทำ dropdown
    ในหน้า Office Scan (ดึงจากข้อมูลจริงหลัง import ไม่ใช่ list ที่ตั้งไว้ล่วงหน้า)"""
    cur = db.cursor()
    cur.execute("""
...
def create_scan(req: ScanLogCreate, db=Depends(get_db), user=Depends(get_current_user)):
    cur = db.cursor()
    cur.execute("SELECT id, department FROM assets WHERE qr_key = %s", (req.qr_key,))
    asset = cur.fetchone()
    if not asset:
        raise HTTPException(status_code=404, detail="Asset not found")
    # Office scan: ถ้า auditor ยืนยัน/แก้แผนกมา ให้เทียบกับแผนกใน master —
    # ต่างกันแปลว่าของถูกย้ายแผนกจริง หรือ master ผิด ต้อง flag ให้ HQ ตรวจ
    dept_mismatch = False
    if req.scanned_department is not None:
        master_dept = (asset["department"] or "").strip()
        scanned_dept = (req.scanned_department or "").strip()
        if master_dept and scanned_dept and master_dept != scanned_dept:
            dept_mismatch = True
...
def update_scan_by_qr(session_id: int, qr_key: str, req: ScanLogPatchByQR,
    ...
    cur.execute("""
        SELECT sl.id, a.department AS master_department FROM scan_logs sl
        JOIN assets a ON a.id = sl.asset_id
        WHERE sl.session_id = %s AND a.qr_key = %s
    """, (session_id, qr_key))
    row = cur.fetchone()
    ...
    if req.scanned_department is not None:
        fields.append("scanned_department = %s"); values.append(req.scanned_department)
        master_dept = (row["master_department"] or "").strip()
        scanned_dept = (req.scanned_department or "").strip()
...
def _parse_asset_excel(content: bytes, filename: str):
    ...
    def _row_to_dict(values):
        ...
        try:
            ...
            # คอลัมน์แผนก (Office Scan) — เพิ่มต่อท้ายแบบ optional เพื่อไม่ทำลาย
            # compatibility กับไฟล์ Excel รุ่นเก่าที่ยังไม่มีคอลัมน์นี้ (จะเป็น None ไปก่อน)
            department = str(values[12]).strip() if len(values) > 12 and values[12] not in (None, "") else None
    ...
    try:
        for r in rows:
            ...
            cur.execute("""
                INSERT INTO assets
                    (asset_code, seq, qr_key, location_code, location_name, name,
                     ...
                     accumulated_dep, net_book_value, department, import_batch_id, is_active, imported_at)
            ...
            """, (r["asset_code"], r["seq"], r["qr_key"], r["location_code"], r["location_name"],
                  ...
                  r["purchase_price"], r["accumulated_dep"], r["net_book_value"], r["department"], batch_id))
            if cur.rowcount:
                inserted += 1
            else:
                skipped += 1
...
def _build_branded_workbook(req: ExportExcelRequest, exported_by: str):
    ...
    # แถว 1: Wordmark "BATA THAILAND" (ไม่มีไอคอนโลโก้ — ตามแบบที่เลือก: มินิมอล ไม่หนักเกินไป)
    ws.row_dimensions[1].height = 26
    ws["A1"].value = "BATA"
    ws["A1"].font = Font(name="Arial", size=18, bold=True, color=BATA_RED_DARK)
    ws["A1"].alignment = Alignment(horizontal="left", vertical="center")
    ws["B1"].value = "THAILAND"
    ws["B1"].font = Font(name="Arial", size=11, bold=True, color="444444")
    ws["B1"].alignment = Alignment(horizontal="left", vertical="center")

    # แถว 2: ชื่อรายงาน
    ws.merge_cells(f"A2:{last_col_letter}2")
    subtitle_cell = ws["A2"]
    subtitle_cell.value = "Fixed Asset Audit — " + req.title
    subtitle_cell.font = Font(name="Arial", size=11, bold=True, color="333333")
    subtitle_cell.alignment = Alignment(horizontal="left", vertical="center")

    # แถว 3: เส้นแดงบางๆ คั่น (เหมือน border-bottom สีแดงในเวอร์ชัน PDF)
    ws.row_dimensions[3].height = 4
    for c in range(1, n_cols + 1):
        ws.cell(row=3, column=c).fill = PatternFill("solid", fgColor=BATA_RED)
